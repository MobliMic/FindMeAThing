<?php
	try {
		if (!defined('HC_TEST_LOCATION')) {
			define('HC_TEST_LOCATION', dirname(dirname(__FILE__)) . '/.core/');
		}
		require_once(HC_TEST_LOCATION . 'core.class.php');
		...
		$settings['database'] = ['engine'       => 'mysql',
		                         'host'         => '127.0.0.1',
		                         'username'     => 'travis',
		                         'password'     => '',
		...
		$GLOBALS['HC_CORE']->setSite(new HC_Site($settings));
